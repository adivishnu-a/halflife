...
/** A deck reads as a small stack: two edges peek out below the tile. */
export function DeckTile({ deck, newPerDay }: { deck: DeckSummary; newPerDay: number }) {
  ...
  return (
    <li className="relative">
      <div aria-hidden="true" className="stack-ghost two" />
      <div aria-hidden="true" className="stack-ghost one" />
      <div className="stock relative flex flex-wrap items-center gap-x-6 gap-y-3 px-5 py-4 sm:px-6">
        <div className="min-w-0 flex-1">
          <Link href={`/decks/${deck.id}`} className="block break-words text-xl font-bold hover:underline">
            {deck.name}
          </Link>
          <p className="mt-0.5 text-sm muted">
            {deck.due > 0 ? <strong className="font-semibold" style={{ color: "#1b1a17" }}>{deck.due} due</strong> : "nothing due"}
            {" · "}
